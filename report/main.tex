...
Посмотрим Heapdump, чтобы понять, что именно вызывает утечку.
\begin{figure}[H]
    \centering
    \includegraphics[width=1\linewidth]{HeapDump.png}
    \caption{Видно, что утечка происходит из-за того, что в Metaspace накапливаются объекты, которые не удаляются.}
    \label{fig:placeholder}
\end{figure}
    
\begin{figure}[H]
    \centering
    \includegraphics[width=1\linewidth]{ObjectOverview.png}
    \caption{Посмотрим, что это за объекты:}
    \label{fig:placeholder}
\end{figure}
Видно, что утечка находится в массиве элементов, также в это же классе есть метод для очищения объектов, который не вызывается.
\begin{figure}[H]
    \centering
    \includegraphics[width=1\linewidth]{ClearMethod.png}
    \label{fig:placeholder}
\end{figure}
Добавим вызов метода очистки в коде, чтобы исправить утечку.
\begin{figure}[H]
    \centering
    \includegraphics[width=1\linewidth]{AddClearMethod.png}
    \label{fig:placeholder}
\end{figure}
\newpage
В конечном итоге, после добавления вызова метода очистки, утечка памяти была устранена.
\begin{figure}[H]
    \centering
